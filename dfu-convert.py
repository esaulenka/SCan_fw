...
def bytestr(val = ''):
  if sys.version_info > (3, 0):
    # python 3.x
    return bytes(val, encoding='ascii')
  else:
    # python 2.7
    return val
...
def build(file,targets,device=DEFAULT_DEVICE):
  ...
  for t,target in enumerate(targets):
    ...
    tdata = struct.pack('<6sBI255s2I',bytestr('Target'),0,1,bytestr('ST...'),len(tdata),len(target)) + tdata
    data += tdata
  data  = struct.pack('<5sBIB',bytestr('DfuSe'),1,len(data)+11,len(targets)) + data
  v,d=map(lambda x: int(x,0) & 0xFFFF, device.split(':',1))
  data += struct.pack('<4H3sB',0,d,v,0x011a,bytestr('UFD'),16)
